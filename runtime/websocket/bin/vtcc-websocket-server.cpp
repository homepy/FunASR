...
void WebSocketServer::on_message(websocketpp::connection_hdl hdl,
  ...
  switch (msg->get_opcode()) {
    case websocketpp::frame::opcode::text: {
      ...
      if (jsonresult.contains("ucid")) {
        msg_data->msg["ucid"] = jsonresult["ucid"];
      }
    ...
    case websocketpp::frame::opcode::binary: {
      ...
      if (isonline) {
        ...
        msg_data->frame_count++;  // increment frame counter

        // trigger ASR when frame count >= 1000
        if (msg_data->frame_count >= 50 && msg_data->hotwords_embedding != nullptr) {
          LOG(INFO) << "Triggering ASR: frame_count=" << msg_data->frame_count;
          std::vector<std::vector<float>> hotwords_embedding_(*(msg_data->hotwords_embedding));
          asio::post(io_decoder_,
                      std::bind(&WebSocketServer::do_decoder, this,
                                std::move(*(sample_data_p.get())),
                                ...
                                msg_data->msg["svs_itn"]));
          msg_data->msg["access_num"]=(int)(msg_data->msg["access_num"])+1;
          msg_data->frame_count = 0;  // reset frame counter
          sample_data_p->clear();  // clear buffer for next chunk
        }
      }
      break;
    }
